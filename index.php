...
use Gmg\Events\Controllers\PublicHomeController;
use Gmg\Events\Core\SchemaGuard;

try {
    SchemaGuard::requireTables(['events', 'event_images', 'homepage_counters', 'business_partners']);
    (new PublicHomeController())->index();
} catch (Throwable $exception) {
    error_log('Homepage schema preflight failed: ' . $exception->getMessage());
    ...
    echo '<p>Import <strong>database/homepage_admin_upgrade.sql</strong> into the <strong>gmigroup</strong> database, then reload the homepage.</p>';
    echo '</div></body></html>';
}
