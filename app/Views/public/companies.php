...
try {
    ...
    error_log('Footer contact load failed on companies page: ' . $exception->getMessage());
...
try {
    $companiesPageNameSetting = (
        new \Gmg\Events\Models\PageBreadcrumb(
            \Gmg\Events\Core\Database::connection()
        )
    )->get('companies', 'Our Companies');

    $companiesPageName = trim(
        (string) ($companiesPageNameSetting['page_name'] ?? 'Our Companies')
    );

    if ($companiesPageName === '') {
        $companiesPageName = 'Our Companies';
    }
} catch (\Throwable $exception) {
    error_log(
        'Page name load failed for companies: '
...
.company-name {
    ...
    border-top: 1px solid rgba(32,54,108,.08);
    background: linear-gradient(145deg, #10243d 0%, #1A2A3A 68%, #20366C 100%);
    text-align: center;
}
.company-name h3 {
    margin: 0;
...
.company-card.company-card-static::after { display: none; }
@media (max-width: 480px) {
    .company-name { min-height: 66px; padding: 15px 58px 15px 16px; }
    .company-name h3 { font-size: 13px; }
}
...
/* =========================================================
   FINAL INDEX.PHP TYPOGRAPHY MATCH
...
/* Same base scale as updated index.php */
body {
    font-size: 18px;
    line-height: 1.7;
}
...
/* Empty-state copy */
.companies-grid > div[style*="grid-column"] {
    font-size: 1.1rem !important;
}

/* Footer - same scale as updated index.php */
...
<section class="breadcrumb-section">
    <div class="container">
        <div class="breadcrumb-area">
            ...
            <h1 class="breadcrumb-title"><?= e($companiesPageName) ?></h1>
            <ul class="breadcrumb-ul">
                <li><a href="/">Home</a></li>
                <li aria-current="page"><?= e($companiesPageName) ?></li>
            </ul>
        </div>
    </div>
</section>
...
<section class="companies-section">
    <div class="container">
        ...
        <div class="companies-grid">
            <?php if ($companies === []): ?>
                <div style="grid-column:1/-1;padding:38px;border:1px solid rgba(32,54,108,.12);border-radius:22px;background:#fff;text-align:center;color:#5b6876;">No companies are currently available.</div>
            <?php endif; ?>
            <?php foreach ($companies as $company): ?>
                <?php
                $link = trim((string) ($company['website_url'] ?? ''));
                $isExternal = preg_match('#^https?://#i', $link) === 1;
                $href = $link === '' ? '' : ($isExternal ? $link : base_url(ltrim($link, '/')));
                ?>
                <?php if ($href !== ''): ?>
                    <a href="<?= e($href) ?>" <?= $isExternal ? 'target="_blank" rel="noopener noreferrer"' : '' ?> class="company-card" aria-label="View <?= e($company['company_name']) ?>">
                <?php else: ?>
                    <article class="company-card company-card-static">
                <?php endif; ?>
                    <div class="company-image"><img src="<?= e(asset_url($company['image_path'])) ?>" alt="<?= e($company['company_name']) ?>" loading="lazy" decoding="async"></div>
                    <div class="company-name"><h3><?= e($company['company_name']) ?></h3></div>
