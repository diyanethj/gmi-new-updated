...
.hero-scrim {
    ...
    background:
        radial-gradient(circle at 18% 70%, rgba(30,87,145,0.16), transparent 40%),
        linear-gradient(90deg, rgba(4,14,26,0.64) 0%, rgba(4,14,26,0.34) 43%, rgba(4,14,26,0.08) 72%, rgba(4,14,26,0.22) 100%),
        linear-gradient(180deg, rgba(4,14,26,0.12) 0%, rgba(4,14,26,0.03) 42%, rgba(4,14,26,0.55) 100%);
}
.hero-content {
...
/* =========================================================
   LARGE TYPOGRAPHY + EXTRA-LARGE PARTNER LOGOS
   Added 2026-08-13
   ========================================================= */

/* Larger overall text scale */
body {
    font-size: 18px;
    line-height: 1.7;
}
...
/* Slogan */
.gmi-slogan-bar .gmi-slogan-text {
    font-size: 1.85rem;
}
.gmi-slogan-bar .gmi-separator {
    font-size: 2rem;
...
/* ===== EB GARAMOND GLOBAL FONT OVERRIDE ===== */
body,
button,
input,
select,
            ...
            <?php foreach ($counters as $counter): ?>
                <div class="single-counter-box">
                    <div class="counter-icon-wrapper">
                        <img src="<?= e(asset_url($counter['icon_path'])) ?>" alt="<?= e($counter['label']) ?>" class="counter-icon" decoding="async" loading="lazy">
                    </div>
                    <span class="counter-number" data-count="<?= e($counter['counter_value']) ?>">0</span>
                    <h3><?= e($counter['label']) ?></h3>
                </div>
            <?php endforeach; ?>
        </div>
    </div>
        ...
        <?php if ($partners !== []): ?>
            <div class="partner-slider-wrapper">
                <div class="partner-track" id="partnerTrack">
                    <?php for ($partnerLoop = 0; $partnerLoop < 2; $partnerLoop++): ?>
                        <?php foreach ($partners as $partner): ?>
                            <?php $partnerUrl = trim((string) ($partner['website_url'] ?? '')); ?>
                            <div class="partner-slide" <?= $partnerLoop === 1 ? 'aria-hidden="true"' : '' ?>>
                                <?php if ($partnerUrl !== ''): ?>
                                    <a href="<?= e($partnerUrl) ?>" target="_blank" rel="noopener noreferrer" aria-label="Visit <?= e($partner['name']) ?> website">
                                <?php endif; ?>
                                <img src="<?= e(asset_url($partner['image_path'])) ?>" alt="<?= $partnerLoop === 1 ? '' : e($partner['alt_text'] ?: $partner['name']) ?>" decoding="async" loading="lazy">
                                <?php if ($partnerUrl !== ''): ?></a><?php endif; ?>
                            </div>
                        <?php endforeach; ?>
                    <?php endfor; ?>
                </div>
            </div>
            <div class="partner-dots" aria-hidden="true">
                <?php foreach ($partners as $index => $partner): ?><span class="<?= $index === 0 ? 'active' : '' ?>"></span><?php endforeach; ?>
            </div>
        <?php else: ?>
            <div style="padding:35px;text-align:center;color:#5b6876;background:#fff;border-radius:20px;">Business partner logos will appear here.</div>
        <?php endif; ?>
    </div>
</section>
...
<section class="section home-blog-section">
    <div class="container">
        ...
        <div class="home-events-grid">
            <?php if ($latestEvents === []): ?>
                <div style="grid-column:1/-1;padding:35px;text-align:center;color:#5b6876;background:#fff;border-radius:20px;">No published events are available.</div>
            <?php endif; ?>

            <?php foreach ($latestEvents as $event): ?>
                <?php $detailsUrl = base_url('event-details.php') . '?slug=' . rawurlencode((string) $event['slug']); ?>
                <article class="home-event-card">
                    <a href="<?= e($detailsUrl) ?>" class="home-event-image" aria-label="View <?= e($event['name']) ?>">
                        <img src="<?= e(asset_url($event['main_image'])) ?>" alt="<?= e($event['name']) ?>" decoding="async" loading="lazy">
                    </a>
                    <div class="home-event-content">
                        <h3><a href="<?= e($detailsUrl) ?>"><?= e($event['name']) ?></a></h3>
                        <div class="home-event-meta">
                            <span><i class="fas fa-calendar-alt"></i> <?= e(format_event_date($event['event_date'])) ?></span>
                            <span><i class="fas fa-user"></i> <?= e($event['company']) ?></span>
                        </div>
                        <div class="home-event-excerpt">
                            <p><?= e(excerpt($event['description'], 185)) ?></p>
                        </div>
                        <a href="<?= e($detailsUrl) ?>" class="home-event-read-more">Read More <i class="fas fa-angle-double-right"></i></a>
                    </div>
                </article>
            <?php endforeach; ?>
        </div>
