...
try {
    $baseTables = ['admins', 'admin_permissions', 'login_attempts', 'audit_logs'];
    $requiredTables = match (true) {
        $action === 'login' => ['admins', 'admin_permissions', 'login_attempts'],
        str_starts_with($action, 'counters') => array_merge($baseTables, ['homepage_counters']),
        str_starts_with($action, 'partners') => array_merge($baseTables, ['business_partners']),
        str_starts_with($action, 'companies') => array_merge($baseTables, ['website_companies']),
        str_starts_with($action, 'about') => array_merge($baseTables, ['about_members', 'about_teams']),
        str_starts_with($action, 'careers') => array_merge($baseTables, ['job_vacancies', 'job_applications']),
        str_starts_with($action, 'events') => array_merge($baseTables, ['events', 'event_images']),
        $action === 'dashboard' => array_merge($baseTables, ['events', 'event_images', 'homepage_counters', 'business_partners', 'website_companies', 'about_members', 'about_teams', 'job_vacancies', 'job_applications']),
        default => $baseTables,
    };
    ...
    echo '<p>Import <strong>database/companies_careers_company_name_upgrade.sql</strong> into the <strong>gmigroup</strong> database, then reload this page.</p></div></body></html>';
    exit;
}
